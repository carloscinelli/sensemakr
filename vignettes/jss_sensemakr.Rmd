...
In this section, we briefly review the omitted variable bias (OVB) framework for sensitivity analysis presented in @cinelli:jrssb2019. This method builds on a scale-free reparameterization of the OVB formula in terms of partial $R^2$ values. Benefits of thie parameterization include:
...
Where ${\bf X}_{-j}$ represents the vector of covariates ${\bf X}$ excluding $X_{j}$. That is, the terms $k_D$ and $k_Y$ represent how strong the confounder $Z$ is relative to observed covariate $X_j$, where "strength" is measured by the partial $R^2$ they explain in the treatment (for $k_D$) and the outcome (for $k_Y$). Note that we will not know $k_D$ or $k_Y$; rather, any assumed values of ($k_D$, $k_Y$) imposes bounds on the resulting confounding and bias. 
...
Specifically, @cinelli:jrssb2019 show that we can rewrite the strength of the confounders as,
\begin{align}
...
These equations allow the investigator to assess the maximum bias that a confounder at most ``k times'' as strong as a particular covariate $X_j$ could cause, showing the relative strength of confounding necessary for bias to have been problematic. Furthermore, if the researcher has domain knowledge to argue that a certain covariate $X_j$ is particularly important in explaining treatment or outcome variation, and that omitted variables cannot explain as much residual variance of $D$ or $Y$ as that observed covariate, these results can be used to set plausible bounds in the total amount of confonding.
...
In this section we illustrate the basic functionality of the package, using the \proglang{R} version first, with a brief review of the \proglang{Stata} version after. Given that sensitivity analysis requires contextual knowledge to be properly interpreted, we illustrate these tools with a real example. Here we use \pkg{sensemakr} to reproduce all results found in Section 5 of @cinelli:jrssb2019, which estimates the effects of exposure to violence on attitudes towards peace, in Darfur. Further details about this application and the data can be found in @hazlett:jcr2019.
...
Where here `center*wealth*political_attitudes` indicates *fully interacted* terms for these three variables. However trying to fit the model `darfur.complete.model` will result in error: none of the variables `center`, `wealth` or `political_attitudes` were measured. We can nevertheless ask questions such as "how strong would these unobserved confounders (or *all* reamaining unobserved confounders) need to be to change our previous conclusions?" Or, given an assumption on how strongly these and other omitted variables relate to the treatment and the outcome, how would including them have changed our inferences regarding the coefficient of `directlyharmed`? We show next how `sensemakr` answer each of these questions. <!--These are precisely the types of questions that can be answered with `sensemakr`.!-->

<!-- CJH: I'm not sure we need this next bit here as a pre-amble; could just explain when we do it. !-->
Additionally, domain knowledge regarding the main determinants of exposure to violence, such as the special role of gender in targetting, can be used to either (i) aid in interpretation of the magnitude of confounding required to change the research conclusions, or (ii) impose limits upon the strength of unobserved confounding. For instance, even if variables such as `wealth` remained as confounders, one could argue that it is unreasonable to expect that they explain more of the variation of exposure to violence than does gender.  We show next how `sensemakr` can leverage such claims to bound the plausible strength of unobserved variables.
...
The print method for `sensemakr` provides the original (unadjusted) estimate along with three summary sensitivity statistics suited for *routine reporting*: (1) the partial $R^2$ of the treatment with the outcome; (2) the robustnuess value (RV) required to reduce the estimate entirely to zero (i.e. $q=1$); and, (3) the RV beyond which the estimate would no longer be statistically distinguishable from zero at the 0.05 level ($q=1$, $\alpha=0.05$). 
...
The sensitivity table offers a useful summary of how robust the current estimate is to unobserved confounding. However, researchers may extend their analysis by visually exploring the whole range of possible estimates that confounders with different strengths could cause. These plots can also represent different bounds on the plausible strength of confounding based on different assumptions on how they compare to observed covariates.  For these, one can use the plot method for `sensemakr`. We begin by examining the default plot type, contour plots for the point estimate.
...
1. A causal interpretation of the research conclusion may be defended by articulating that a confounder with such strength is unlikely. For instance, one could argue that, given the way injuries (the "treatment") occurred, the scope for targeting particular types of individuals was quite limited; aircraft dropped makeshift and unguided bombs and other objects over villages, and militia raided without concern for who they are targeting---the only known major exception to this was targeting gender, which is one of the most visually apparent characteristics of an individual, and was a strong factor in targetting. 

2. Likewise, similar grounds are required to persuasively dismiss a causal interpretation of the result. There are requirements, in terms of (relative) strength, that the hypothesized unobserved confounders need to meet in order to be problematic. For instance, a skeptic can most helpfully articulate (using domain knowledge) why a confounder that explains at least more than twice of the variation of the treatment assignment than the covariate `female` is plausible. Otherwise, that confounder cannot logically account for all the observed association, even in an extreme scenario.

Robustness in the face of confouding is thus claimed to the extent one agrees with the arguments in 1 (which rely on domain knowledge about attacks in Darfur), while a result can be deemed fragile insofar as alternative stories meeting the requirements in 2 can be offered. Sensitivity analyses should not be used to obviate discussions about confouding by engaging in automatic procedures; rather, they should stimulate a more displined, quantitative argument about confounding, in which such statements are made and debated.

### Advanced use
<!-- CJH: Carlos, you had "going further" as the section name. That seemed a bit casual. Though maybe "advanced use is too boring or intimidating. !-->

The functionality demonstrated above will suffice for most users, most of the time. Sometimes, however, more flexibility will be needed, and can be obtained by employing additional functions in the package. The functions most likely to be called upon can be divded into the following categories, 
...
We demonstrate use of these functions below through examples chosen to illustrate important features of this sensitivity analysis.

## Formal versus informal benchmarking: customizing bounds

<!-- ### Background -->

Informal “benchmarking” procedures have been suggested as aids to interpretation for numerous sensitivy analyses. These approaches are usually described as revealing how an unobserved confounder $Z$ “not unlike” some observed covariate $X_j$ would alter the results of a study (e.g., Imbens, 2003; Blackwell, 2013; Hosman et al. 2010, Dorie et al., 2016, Hong et al. 2018). \cite{cinelli:jrssb2019} show why these proposals may lead users to erroneous conclusions, leading to the bounding approach described above.  We provide an example here that illustrates the potential for misleading results from informal benchmarking (replicating Section 6.1 of @cinelli:jrssb2019), which also demonstrate more advanced usage of `sensemakr` including how users can construct their own sensitivity contour plots with customzed bounds. <!-- To demonstrate more advanced use of the `sensemakr` package while helping to illustrate the risk of informal benchmarking approaches, we replicate here the example from Section 6.1 of  @cinelli:jrssb2019, in which such benchmarks produce clearly misleading results. Replicating this example with `sensemakr` provides a useful tutorial on how users can construct their own sensitivity contour plots with customized bounds, beyond what is offered by default on the package.!-->
...
Also note that, in this model: (i) the unobserved confounder $Z$ is independent of $X$; and, (ii) the unobserved confounder $Z$ is *exactly like* $X$ in terms of its strength of association with the treatment and the outcome. The code below creates a sample of size 100 of this data generating process. We use the function `resid_maker()` to make sure the residuals are standardized and orthogonal, thus all properties that we describe here hold exactly even in any finite sample example.
...
Let us suppose the investigator *correctly* knows that: (i) $Z$ and $X$ have the same strength of association with $D$ and $Y$; and, (ii) $Z$ is independent of $X$. How can she leverage this information to undersand how much bias a confounder $Z$ "not unlike" $X$ could cause?As we have seen in Section XXX, it is possible to use the observed partial $R^2$ of $X$ with $D$ and $Y$ with Equation \ref{eq:bounds} to obtain valid bounds on the amount of confounding caused by an unobserved $Z$ as strongly associated with the treatment and with the outcome as $X$.  
...
As we can see, the results of the bounding procedure shows us that, an unobserved confounder $Z$ that is truly "not unlike $X$"  would: (1) explain 50% of the residual variation of the treatment and 33% of the residual variation of the outcome; (2) bring the point estimate exactly to zero; and, (3) bring the standard error to 0.102. And this is precisely what one obtains when running the full regression model adjusting for \emph{both} $X$ and $Z$, as shown in the second column of Table \ref{tab:naive}.

### Informal benchmarks

This section demonstrates an ``informal benchmark'' to show its dangers. Computing the bias due to the omission of $Z$ requires two sensitivity parameters: its partial $R^2$ with the treatment $D$ and its partial $R^2$ with the outcome $Y$.  Informal approaches follow from the intuition that, rather than work with Equation \ref{eq:bounds}, we can simply take the observed partial $R^2$ of $X$ with $D$ and $Y$ found directly from regressions for the treatment and outcome, repsectively. Unfortunately, as formalized in \cite{cinelli:jrssb2019}, these observed associations are themselves biased, making comparisons using them potentially misleading. <!-- from those regressions, as plausible values for the sensitivity parameters. Such is the rationale is the essence of many informal benchmarking proposals. However, 
But what can happen if one uses this informal approximation insted of the formal bounds?!--> What happens if we nevertheless attempt to use those observed statistics for benchmarking?  

To compute the informal benchmarks, we first need to obtain the observed partial $R^2$ of $X$ with $Y$. This can be done using the `partial_r2()` function of `sensemakr` in the `model.ydx` regression.
...
We next need to obtain the partial $R^2$ of $X$ with the treatment. For that, we need to fit a new regression of the treatment $D$ on the observed covariate $X$.
...
Let us now comapre the informal benchmarks with the previous formal bounds. First one should plot the sensitivity contours with the function `ovb_contour_plot()`. Next, we add the informal benchmark to the contours, using the numeric method of the function `add_bound_to_contour()`. Finally, the formal bounds previously computed are added. When needed, users can customize the position of the bound label in the plot with the arguments `label.bump.x` and `label.bump.y`. <!--Several other graphical customizations are available.!-->
...
Note the results of the informal benchmark are very different from what one expects. The informal benchmark point is still far away from zero, and this would lead an investigator to incorrectly conclude that an unobserved confounder $Z$ "not unlike $X$" is not sufficient to explain away the observed effect, when in fact it would. Moreover, this incorrect conclusion occurs despite the investigator *correctly* assuming both that: (i) $Z$ and $X$ have the same strength of association with $D$ and $Y$; and, (ii) $Z$ is independent of $X$. 

<!-- CJH: I considered an approach in which the "explanation" is much more cursory and comes earlier in this section as motivation, for fear that this longer explanation is unnecessary to a software paper.  But happy to discuss. 

Why does this happen? As explained in Section 6.1 of @cinelli:jrssb2019, there are two problems affecting informal benchmarks in this setting. First, we have to make an adjustment of baseline variance to be explained, since the sensitivity parameters consider the partial $R^2$ of $Z$ with the outcome, after taking into account what is already explained by $X$. Second, consider the DAG of our structural model, as shown in Figure \ref{fig:dag_naive}. Note that, although $Z$ is *marginally* independent of $X$, note that $Z$ is not *conditionally* independent of $X$, *given* $D$, because $D$ is a *collider* (Pearl, 2009). Conditioning on $D$, thus, creates an association between $Z$ and $X$, and this distorts the observed quantities of $X$ that are being used for benchmarking. Finally, note that using the formal bounds leads the researcher to the correct conclusion: an unobserved confounder $Z$ similar to $X$ is strong enough to explain away all the observed association. Given these considerations, we do not recomment using informal benchmarks for sensitivity analysis, and suggest empirical researchers to use formal approaches such as the ones provided with `ovb_bounds()`.  For further details and discussion, we point readers to Sections 4.4 and 6.1 of @cinelli:jrssb2019.

\begin{figure}
\centering
\includegraphics[width = 2.5in, height = 2.5in]{collider.png}
\caption{DAG of the model given by Equations \ref{eq:naive_first} to \ref{eq:naive_last}. Note that, although,  $Z$ is marginally independent of $X$, conditioning on $D$ creates dependency between these two variables, since $D$ is a collider (Pearl, 2009).}
\label{fig:dag_naive}
\end{figure}
!-->

## Assessing the sensitivity of existing regression results
We conclude this section by demonstrating how summary regression output can be used for sensitivity analysis without access to the underlying data, replicating all of the results from Section XXX above now using only results from the regression table.  
<!--We end this section by showing users how they can assess the sensitivity of results they see in print without requiring a reanalysis of the underlying data. To illustrate, we show how one can reproduce all the results Secion XXX with only the summary statistics from the regression table.!-->

### Sensitivity statistics

The robustness value and partial $R^2$ are key sensitivity statistics, useful for standard reporting and with direct interpretations. These can be computed directly by the user with the functions <!-- If the user wants to just compute those, instead of using `sensemakr()`, it may be simpler to use the individual functions for each of the sensitivity statistics, namely,!-->\code{robustness_value()} and \code{partial_r2()}. These functions have both \code{lm} and \code{numeric} numeric methods so that users may pass in an ``lm` object or call them using numerical values based from elsewhere. With a fitted \code{lm} model in hand, the most convenient way to compute the RV and partial $R^2$ is by
...
\noindent However, when one does not have access to the data in order to run this model, simple summary statistiscs such as the point estimate for the `directlyharmed`  (0.097), its standard error (`0.023`) and the degress of freedom of the regression (`783`) are sufficient to compute the RV and partial $R^2$ using numeric methods,
```{r rv_num, results='hide'}
robustness_value(t_statistic = 0.097/0.023, dof = 783)
partial_r2(t_statistic = 0.097/0.023, dof = 783)
```

The package `sensemakr` also provides a convenience function \code{sensitivity_stats()} that computes all sensitivity statistics for a regression coefficient of interest and returns them in a \code{data.frame}.
...
The extreme scenario plots (as in Figure \ref{fig:extreme}) can also be reproduced from summary statistics using the function `ovb_extreme_plot()`, <!--function it is also possible to reproduce Figure \ref{fig:extreme} with the summary statistics.!-->

```{r extreme_numeric, eval = FALSE}
ovb_extreme_plot(estimate = 0.097, se = 0.023, dof = 783)
```

All plotting functions return invisibly the data needed to reproduce them, thereby allowing users that prefer to create their own plots to easily do so.

### Adjusted estimates, standard errors and t-values 

These functions allow users to compute the adjusted estimates given different postulated degrees of confounding. As before, they can be called directly with `lm` objects or numerical data. For instance, in the case of Darfur, suppose a researcher has reasons to believe a confounder explains 10% of the residual variance of the treatment and 20% of the residual variance of the outcome.  Without the underlying data, she can still compute the bias that a confounder with such strength could cause. Here we show how to obtain the adjusted estimate and t-value that one would have obtained in the full regression adjusting for such confounder.
...
<!-- CJH: by here I'm worried the reader will get really bored of all this. !-->
...
```{r r2_female}
r2yxj.x <- partial_r2(t_statistic = -0.232/0.024, dof = 783)
...
Finally, one uses the `adjusted_estimate()` function to compute the estimates implied by such hypothetical confounders.
...
We can use all this information along with the numeric methods of the plot functions to reproduce the contour plots of Figure \ref{fig:darfur_contours} using only summary statistics usually found in regression tables. Note that, since we are performing all calculations manually, appropriate limits of the plot area also need to be set by the user.
