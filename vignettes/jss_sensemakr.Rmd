...
Computing the bias due to the omission of $Z$ requires two sensitivity parameters: its partial $R^2$ with the treatment $D$ and its partial $R^2$ with the outcome $Y$.  Intuitively, it seems that, instead of appealing to  Equation \ref{eq:bounds}, one could take a simpler route: take as references the observed partial $R^2$ of $X$ with $D$ and $Y$, and use those, without adjustment, as the plausible values for the sensitivity parameters. This rationale is the essence of many informal benchmarking proposals. But what can happen if one uses this informal approximation insted of the formal bounds?
...
Once both partial $R^2$ are computed, we can determine what would be the implied adjusted estimate due to an unobserved confounder $Z$ with this pair of partial $R^2$. This can be computed using the `adjusted_estimate()` function. 
...
We can now compare the informal benchmarks with the previous formal bounds. First one should plot the sensitivity contours with the function `ovb_contour_plot()`. Next, we add the informal benchmark to the contours, using the numeric method of the function `add_bound_to_contour()`. Finally, the formal bounds previously computed are added. When needed, users can customize the position of the bound label in the plot with the arguments `label.bump.x` and `label.bump.y`. Several other graphical customizations are available.
...
## Revisiting Darfur: assessing the sensitivity of results you see in print

We end this section by showing users how they can assess the sensitivity of results they see in print, but do not have access to microdata. To this end, we now take a more practical approach and show how one can reproduce all the results Secion XXX with only the summary statistics from the regression table.


### Sensitivity statistics

The robustness value and partial $R^2$ have been described above as key sensitivity statistics useful in standard reporting and with direct interpretations. If the user wants to just compute those, instead of using `sensemakr()`, it may be simpler to use the individual functions for each of the sensitivity statistics, namely, \code{robustness_value()} and \code{partial_r2()}. These functions have both \code{lm} and \code{numeric} numeric methods so that users may pass in an \code{lm} object or call them using numerical values based from elsewhere. The code below computes the RV and the partial $R^2$ directly from the regression model.
...
When one does not have access to the data, but has access to summary statistiscs such as the point estimate for the `directlyharmed`  (0.097), its standard error (`0.023`) and the degress of freedom of the regression (`783`), on can still compute the RV and partial $R^2$.
...
The package `sensemakr` also provides a convenience function \code{sensitivity_stats()} that computes all sensitivity statistics for a regression coefficient of interest. The function returns a \code{data.frame} with those values.
...
With the `ovb_extreme_plot()` function it is also possible to reproduce Figure \ref{fig:extreme} with the summary statistics.
...
Finally, we show how users can compute bounds on the strength of confounding using only summary statistics, if the paper also provides a *treatment* regression table, i.e., a regression of the treatment on the observed covariates. These regressions are usually performed for "balance tests", in which the investigator assesses whether observed covariates predict treatment assigment. For the Darfur example, this regression is shown in Table \ref{tab:treat_reg}.
...
We can use all this information along with the numeric methods of the plot functions to reproduce the contour plots of Figure \ref{fig:darfur_contours} using only summary statistics usually found in regression tables. Note that, since we  are performing all calculations manually, the proper limits of the plot area also need to be set by the user.
