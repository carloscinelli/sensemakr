...
This section demonstrates an ``informal benchmark'' to show its dangers. Computing the bias due to the omission of $Z$ requires two sensitivity parameters: its partial $R^2$ with the treatment $D$ and its partial $R^2$ with the outcome $Y$.  Informal approaches follow from the intuition that we can simply take the observed partial $R^2$ of $X$ with $D$ and $Y$ found directly from regressions for the treatment and outcome, respectively. Unfortunately, as formalized in \cite{cinelli:jrssb2019}, these observed associations are themselves biased, making comparisons using them potentially misleading. The relationships in Equation \ref{eq:bounds} allow us, however, to move from the observed associations in those regressions to the bounds on the partial $R^2$ values of interest.<!-- from those regressions, as plausible values for the sensitivity parameters. Such is the rationale is the essence of many informal benchmarking proposals. However, 
But what can happen if one uses this informal approximation insted of the formal bounds?!--> 

What happens if we nevertheless attempt to use those observed statistics for benchmarking? To compute the informal benchmarks, we first need to obtain the observed partial $R^2$ of $X$ with $Y$. This can be done using the `partial_r2()` function of `sensemakr` in the `model.ydx` regression.
...
Let us now compare those informal benchmarks with the formal bounds describes above. To prepare, we first plot sensitivity contours with the function `ovb_contour_plot()`. Next, add the informal benchmark to the contours, using the numeric method of the function `add_bound_to_contour()`. Finally, we can add the formal bounds computed above. <!-- When needed, users can customize the position of the bound label in the plot with the arguments `label.bump.x` and `label.bump.y`. <!--Several other graphical customizations are available.!-->
...
Note the results from informal benchmarking are misleading here: even though $Z$ and $X$ are exactly alike in their roles,  the informal benchmark point is still far from zero, which would suggest that an unobserved confounder $Z$ "not unlike $X$" is unable to explain away the observed effect, when in fact we know it would explain it away in this example. This incorrect conclusion occurs despite the investigator *correctly* assuming both that: (i) $Z$ and $X$ have the same strength of association with $D$ and $Y$; and, (ii) $Z$ is independent of $X$. For further details and discussion, we point readers to Sections 4.4 and 6.1 of @cinelli:jrssb2019.

<!-- CJH: I think this explanation is great and would be perfect in a vignette, but it comes across as much more explainy than what we've been doing throughout the rest of this, and I'd lean towards keeping it out of the software paper. I've instead given a very broad suggestion at the top of this section as to why you can't use the raw observed associations. I've also takent the final sentence below, pointing readers to the paper for further details and discussion, and put it above.
...
\noindent However, when one does not have access to the data in order to run this model, simple summary statistiscs such as the point estimate for the `directlyharmed`  (0.097), its standard error (`0.023`) and the degress of freedom of the regression (`783`) are sufficient to compute the RV and partial $R^2$ using numeric methods, as in
...
These functions allow users to compute the adjusted estimates given different postulated degrees of confounding. <!-- As before, they can be called directly with `lm` objects or numerical data. For instance, in the case of Darfur, suppose a researcher has reasons to believe a confounder explains 10% of the residual variance of the treatment and 20% of the residual variance of the outcome.!--> If the underlying data are not available, the investigator can still compute the bias that a confounder with such strength could cause. Here we show how to obtain the estimate and t-value that one would have obtained in the full regression adjusting for such confounder:
...
This information along with the numeric methods for the plot functions, allow us to reproduce the contour plots of Figure \ref{fig:darfur_contours} using only summary statistics. Note that, since we are performing all calculations manually, appropriate limits of the plot area also need to be set by the user.
