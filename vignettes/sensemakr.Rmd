...
Note that because no `benchmark` argument was not set, `sensemakr` defaults to using all covariates as benchmarks. For our next task, it will be easier to use only a subsect of the benchmarks: 
...
### The `sensemakr` object

The object, `sense.out`, is not usually inspected directly, but contains many valuable quantities.  

sense <- sensemakr(model=model, treatment="directlyharmed", benchmarks=X)


Suppose now we are interested in a subset of these covariates 
