...
1) Fit a linear outcome model via `lm()`. This should have your treatment and your (pre-treatment) covariates on the right hand side.   
2) Create a sensemakr object, which contains the necessary sensitivity quantitites, using `sensemakr()`.
3) Explore the results by use of `plot()` and `summary()`.
