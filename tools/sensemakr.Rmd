...
```{r}
lm.full  <- lm(peacefactor ~ directlyharmed + age + farmer_dar + herder_dar +
...
The \textit{worst case scenario} considers the sensitivity of results based on the unexplained variance. The worst case is one in which all unexplained variation in the outcome belongs to an omitted confounder. How strongly would such an omitted variable need to be associated with the treatment in order to reduce the treatment effect estimate by some amount -- say 100\%?
